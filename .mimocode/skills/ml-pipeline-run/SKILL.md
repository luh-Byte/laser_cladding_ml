...
```powershell
.venv\Scripts\python.exe -m src.fewshot_finetune
```
- Duration: ~1min
...
```powershell
.venv\Scripts\python.exe -m src.pareto_optimization
```
- Duration: ~2min (375,000 samples)
- Outputs: pareto_global_front.csv, representative_params.csv

```powershell
.venv\Scripts\python.exe -m src.fewshot_pareto
```
- Duration: ~2min
- Outputs: fewshot pareto results

```powershell
.venv\Scripts\python.exe -m src.loo_validation
```
- Duration: ~2min
- Outputs: loo_results.csv, loo_metrics_summary.csv

```powershell
.venv\Scripts\python.exe -m src.validate_experiment
```
- Duration: ~30s
...
```powershell
.venv\Scripts\python.exe -m src.figures_main
```
- Duration: ~30s
- Outputs: 19 figures (01-19_*.png)

```powershell
.venv\Scripts\python.exe -m src.figures_extra
```
- Duration: ~30s
- Outputs: 6 figures (14-19_*.png)

```powershell
.venv\Scripts\python.exe -m src.figures_shap
```
- Duration: ~30s
...
```powershell
.venv\Scripts\python.exe -c "import sys; sys.path.insert(0, '.'); from src.figures_main import plot_stacked_composition, load_all_data; data = load_all_data(); plot_stacked_composition(data)"
```

Replace `plot_stacked_composition` with the function name from `figures_main.py`, `figures_extra.py`, or `figures_shap.py`.
